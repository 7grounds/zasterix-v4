...
export default function OrigoChat() {
  ...
  // Statische Konstante verhindert ESLint 'unused-vars' Fehler
  ...
  return (
    <div className="flex flex-col h-screen bg-[#020617] text-slate-300 font-mono text-sm">
      <header className="p-4 border-b border-slate-800 bg-black flex justify-between items-center">
        <span className="text-xs font-bold tracking-[0.3em] text-sky-500 uppercase">Origo V4 // Stable</span>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        {loading && <div className="max-w-4xl mx-auto text-[10px] animate-pulse text-sky-500 uppercase">Processing...</div>}
        <div ref={scrollRef} />
      </div>
      ...
      <footer className="p-6 bg-black border-t border-slate-900">
        <form onSubmit={sendMessage} className="flex gap-4 max-w-4xl mx-auto">
          <input 
            ...
            disabled={loading}
          />
          <button type="submit" disabled={loading} className="text-xs font-black uppercase text-sky-500 hover:text-white">
            Execute
          </button>
        </form>
      </footer>
