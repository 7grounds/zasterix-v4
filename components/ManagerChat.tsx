...
export default function ManagerChat() {
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    if (cmd.toLowerCase().includes('session') && !activeLeader) {
      ...
      if (leader) {
        const leaderData = leader as any; // FIX: Cast to any to satisfy TypeScript
        ...
        return; // Handle initial summon first
      }
    }
  ...
  return (
    <div className="flex flex-col h-screen bg-white overflow-hidden">
      <div className="px-10 py-6 border-b border-slate-50 flex justify-between bg-white/80 backdrop-blur-md sticky top-0">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-400">
            {activeLeader ? `${(activeLeader as any).name} | ROUND ${round}/3` : 'MANAGER_ALPHA'}
          </span>
        </div>
      </div>
      ...
      <div className="p-10 md:p-16 bg-white border-t border-slate-50">
        <form onSubmit={handleAction} className="max-w-6xl mx-auto relative">
          <input 
            ...
            placeholder={activeLeader ? "Contribute..." : "Command..."} 
