...
import { supabase } from '../lib/supabase'; // Korrigierter Pfad: Raus aus components, rein in lib
...
export default function AgentList() {
  ...
  if (loading) return <div className="p-10 text-gray-400 italic font-mono">SYNCHRONISIERE_ORIGO_KERN...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {agents.map((agent) => (
          <div key={agent.id} className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm">
            ...
            <p className="text-xs text-slate-400 mt-1 uppercase tracking-tight">{agent.category || 'Standard'}</p>
          </div>
        ))}
      </div>
    </div>
